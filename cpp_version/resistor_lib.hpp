...
namespace ResistorLib {

    // Valori commerciali standard (IEC 60063)
    const std::map<std::string, std::vector<double>> e_series = {
        {"E3", {1.0, 2.2, 4.7}},
    ...
    // Package e potenze (valori tipici)
    const std::map<std::string, double> package_power = {
        {"0201", 0.05}, {"0402", 0.0625}, {"0603", 0.1}, {"0805", 0.125},
        {"1206", 0.25}, {"1210", 0.33}, {"1812", 0.5}, {"2010", 0.75},
        {"2512", 1.0}, {"AXIAL", 0.25}, {"RADIAL", 0.25}
    };
    ...
    const std::map<std::string, ColorInfo> color_codes = {
    ...
    const std::map<std::string, ToleranceInfo> tolerance_colors = {
        {"marrone", {1.0, "#8B4513"}}, {"rosso", {2.0, "#FF0000"}}, {"verde", {0.5, "#008000"}},
        {"blu", {0.25, "#0000FF"}}, {"viola", {0.1, "#800080"}}, {"grigio", {0.05, "#808080"}},
        {"oro", {5.0, "#FFD700"}}, {"argento", {10.0, "#C0C0C0"}}, {"nessuno", {20.0, "#D3D3D3"}}
    };

    // Funzione di utilità per formattare il valore
    std::string format_value(double value) {
        char buffer[100];
        if (value >= 1e6) {
            snprintf(buffer, sizeof(buffer), "%.3f MΩ", value / 1e6);
        } else if (value >= 1e3) {
            snprintf(buffer, sizeof(buffer), "%.3f kΩ", value / 1e3);
        } else if (value >= 1) {
            snprintf(buffer, sizeof(buffer), "%.3f Ω", value);
        } else {
            snprintf(buffer, sizeof(buffer), "%.3f mΩ", value * 1e3);
        }
